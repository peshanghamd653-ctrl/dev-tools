...
- **The e2e suite does not run in CI.** Everything else does —
  `.github/workflows/ci.yml` runs fmt/clippy/`cargo test --workspace` on
  `windows-latest` (deliberately: `devos-terminal`'s ConPTY tests are
  `#[cfg(all(test, windows))]`, so an Ubuntu run would report green having
  never compiled them), typecheck/lint/test/build on Ubuntu, and an
  advisory audit weekly as well as per-push. The blocker for e2e is not the
  Windows requirement but that the app hardcodes its database to
  `app_data_dir()/devos.db`, so a CI run would have nowhere isolated to
  boot. See [release-process.md](release-process.md).
